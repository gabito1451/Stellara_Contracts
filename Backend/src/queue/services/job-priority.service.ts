...
import { JobPriority, JobPriorityLevel, PRIORITY_WEIGHTS } from '../types/enhanced-job.types';
...
export class JobPriorityService {
  ...
  private staleJobs: Map<string, { addedAt: Date; priority: JobPriority }> = new Map();
  
  // Resource allocation tracking
  private resourceAllocation: Map<string, { allocated: number; used: number }> = new Map();

  /**
   * Get priority weight for a job priority level
   */
  getPriorityWeight(priority: JobPriority): number {
    return PRIORITY_WEIGHTS[priority.level] || PRIORITY_WEIGHTS[JobPriorityLevel.NORMAL];
  }
  ...
  determineJobPriority(
    jobName: string,
    jobData: any,
    metadata?: { tags?: string[]; priority?: JobPriority; parentId?: string; correlationId?: string }
  ): JobPriority {
  ...
  applyDynamicAdjustment(basePriority: JobPriority): JobPriority {
    ...
    const adjustedWeight = Math.round(basePriority.weight * this.systemLoadFactor);
   ...
   */
  inheritPriorityFromParent(childPriority: JobPriority, parentId: string): JobPriority {
   ...
   * Check for stale job escalation
   */
  checkStaleJobEscalation(jobName: string, basePriority: JobPriority): JobPriority {
    ...
    if (basePriority.level === JobPriorityLevel.LOW && 
  ...
  registerDependency(dependency: JobDependency): void {
    const queueDeps = this.jobDependencies.get(dependency.dependentQueueName) || [];
    queueDeps.push(dependency);
    this.jobDependencies.set(dependency.dependentQueueName, queueDeps);
    
    this.logger.log(`Registered dependency: ${dependency.jobId} -> ${dependency.dependentJobId}`);
  }
  ...
  allocateResources(jobId: string, queueName: string, amount: number): boolean {
    ...
    const allocation = this.resourceAllocation.get(key) || { allocated: 0, used: 0 };
    
    // Check if we have enough available resources
    const available = allocation.allocated - allocation.used;
    if (available < amount) {
      this.logger.warn(`Insufficient resources for job ${jobId}. Requested: ${amount}, Available: ${available}`);
      return false;
    }
    ...
    this.logger.log(`Allocated ${amount} resources for job ${jobId} in queue ${queueName}`);
    return true;
  }
  ...
  releaseResources(jobId: string, queueName: string, amount: number): void {
    ...
    if (allocation) {
      ...
      this.logger.log(`Released ${amount} resources for job ${jobId} in queue ${queueName}`);
    }
  }
  ...
  setResourceAllocation(queueName: string, totalAmount: number): void {
    const key = `${queueName}_resources`;
    const current = this.resourceAllocation.get(key) || { allocated: 0, used: 0 };
    current.allocated = totalAmount;
    this.resourceAllocation.set(key, current);
    
    this.logger.log(`Set resource allocation for queue ${queueName} to ${totalAmount}`);
  }

  /**
   * Get resource utilization for a queue
   */
  getResourceUtilization(queueName: string): { used: number; allocated: number; utilization: number } {
    const key = `${queueName}_resources`;
    const allocation = this.resourceAllocation.get(key) || { allocated: 0, used: 0 };
    
    const utilization = allocation.allocated > 0 ? allocation.used / allocation.allocated : 0;
    return {
  ...
  private getPriorityByJobType(jobName: string, jobData: any): JobPriority {
    switch (jobName) {
      ...
      default:
        return { level: JobPriorityLevel.NORMAL, weight: PRIORITY_WEIGHTS[JobPriorityLevel.NORMAL] };
    }
  }
  ...
  private getContractDeploymentPriority(jobData: any): JobPriority {
    ...
    if (jobData.environment === 'production') {
      return { level: JobPriorityLevel.CRITICAL, weight: PRIORITY_WEIGHTS[JobPriorityLevel.CRITICAL] };
    }
    
    // High priority for urgent deployments
    if (jobData.urgent || jobData.priority === 'high') {
      return { level: JobPriorityLevel.HIGH, weight: PRIORITY_WEIGHTS[JobPriorityLevel.HIGH] };
    }
    ...
    return { level: JobPriorityLevel.LOW, weight: PRIORITY_WEIGHTS[JobPriorityLevel.LOW] };
  }
  ...
  private getMarketNewsPriority(jobData: any): JobPriority {
    ...
    if (jobData.timestamp) {
      ...
      if (newsAge < 300000) { // Less than 5 minutes
        return { level: JobPriorityLevel.HIGH, weight: PRIORITY_WEIGHTS[JobPriorityLevel.HIGH] };
      }
    }
   ...
   */
  private adjustPriorityByTags(priority: JobPriority, tags: string[]): JobPriority {
    let adjustedPriority = { ...priority };

    // Tags that increase priority
    const highPriorityTags = ['urgent', 'critical', 'real-time', 'production', 'breaking'];
    const criticalTags = ['emergency', 'security', 'compliance'];

    // Tags that decrease priority
    const lowPriorityTags = ['batch', 'bulk', 'test', 'development', 'low-priority'];

    // Check for critical tags first
    if (criticalTags.some(tag => tags.includes(tag))) {
      adjustedPriority = { level: JobPriorityLevel.CRITICAL, weight: PRIORITY_WEIGHTS[JobPriorityLevel.CRITICAL] };
    }
    // Then check for high priority tags
    else if (highPriorityTags.some(tag => tags.includes(tag))) {
      adjustedPriority = { level: JobPriorityLevel.HIGH, weight: PRIORITY_WEIGHTS[JobPriorityLevel.HIGH] };
    }
    // Then check for low priority tags
    else if (lowPriorityTags.some(tag => tags.includes(tag))) {
      adjustedPriority = { level: JobPriorityLevel.LOW, weight: PRIORITY_WEIGHTS[JobPriorityLevel.LOW] };
    }
  ...
  getPriorityDistribution(jobs: any[]): Record<JobPriorityLevel, number> {
    ...
    jobs.forEach(job => {
      const priority = job.opts?.priority || PRIORITY_WEIGHTS[JobPriorityLevel.NORMAL];
