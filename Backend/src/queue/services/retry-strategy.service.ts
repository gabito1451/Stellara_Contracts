...
export class RetryStrategyService {
   ...
   */
  shouldRetry(error: Error, attempt: number, strategy: RetryStrategy, queueName?: string): boolean {
    // Check circuit breaker state if queueName is provided
    if (queueName && this.isCircuitBreakerOpen(queueName, strategy.circuitBreakerTimeout || 60000)) {
      this.logger.warn(`Circuit breaker open for queue ${queueName}, skipping retry`);
      return false;
    }
    ...
    if (!shouldRetry) {
      ...
      this.logger.warn(`Non-retryable error ${error.name} (${errorType}), skipping retries`);
      return false;
    }
  ...
  private evaluateErrorForRetry(errorType: string, error: Error): boolean {
    ...
    if (permanentErrors.some(err => error.name === err || error.message.includes(err))) {
      return false;
    }

    if (transientErrors.some(err => error.name === err || error.message.includes(err))) {
      return true;
    }

    if (conditionalErrors.some(err => error.name === err || error.message.includes(err))) {
  ...
  private classifyError(error: Error): string {
    ...
    if (error.message.includes('5xx') || error.message.includes('server error')) {
      return 'ServerError';
    }
   ...
   */
  async executeRetryCallback(strategy: RetryStrategy, attempt: number, error: Error): Promise<void> {
    ...
    const defaultStrategies: Record<string, RetryStrategy> = {
      'deploy-contract': {
        ...
        retryCallback: async (attempt, error) => {
          console.log(`Deploy contract retry attempt ${attempt} due to: ${error.message}`);
        }
      },
      'process-tts': {
        type: 'exponential',
        ...
        retryCallback: async (attempt, error) => {
          console.log(`TTS process retry attempt ${attempt} due to: ${error.message}`);
        }
      },
      'index-market-news': {
        type: 'linear',
        ...
        circuitBreakerTimeout: 300000, // 5 minutes
        retryCallback: async (attempt, error) => {
          console.log(`Market news index retry attempt ${attempt} due to: ${error.message}`);
        }
      }
    };

    return defaultStrategies[jobType] || {
      type: 'exponential',
      delay: 3000,
      maxAttempts: 3,
      backoffMultiplier: 2,
  ...
  private isCircuitBreakerOpen(queueName: string, timeoutMs: number): boolean {
    ...
    if (state.state === 'OPEN') {
      ...
      if (timeSinceLastFailure > timeoutMs) {
        ...
        this.logger.log(`Circuit breaker for ${queueName} transitioning to HALF_OPEN`);
  ...
  private recordFailure(queueName: string): void {
    ...
    if (state.failureCount >= 5 && state.state !== 'OPEN') { // Default threshold
      state.state = 'OPEN';
  ...
  recordSuccess(queueName: string): void {
    ...
    if (state) {
      if (state.state === 'HALF_OPEN') {
        this.logger.log(`Circuit breaker for ${queueName} closed after successful retry`);
      }
  ...
  private calculateExponentialDelay(strategy: RetryStrategy, attempt: number): number {
  ...
  private calculateLinearDelay(strategy: RetryStrategy, attempt: number): number {
    const baseDelay = strategy.delay;
  ...
  private calculateFibonacciDelay(strategy: RetryStrategy, attempt: number): number {
  ...
  private calculateJitterDelay(strategy: RetryStrategy, attempt: number): number {
